  ...
  public void toggleDownload(String name, Uri uri, String extension) {
    if (isDownloaded(uri)) {
      ...
      StartDownloadDialogHelper helper =
          new StartDownloadDialogHelper(getDownloadHelper(uri, extension), name);
      helper.prepare();
    }
  }

  void startDownload(String entryId, Uri uri) {
    final DownloadHelper downloadHelper = getDownloadHelper(uri, null);

    StartDownloadDialogHelper helper =
            new StartDownloadDialogHelper(getDownloadHelper(uri, null), entryId);
    helper.prepare();
  }
