...
EOF
    ...
    if [ "$RELEASE_TYPE" = "Update" ]; then
                      JSON_BODY="### Kaltura Player Plugin Support\n\n"
                      JSON_BODY="$JSON_BODY$NEW_TAG\n\n"
          JSON_BODY="$JSON_BODY * upgrade to $NEW_TAG\n\n"
          JSON_BODY="$JSON_BODY #### Gradle\n\n"
                      JSON_BODY="$JSON_BODY * implementation 'com.kaltura.player:tvplayer"
      ...
      "sections": [
          {
              "facts": [
                  ...
                  {
                      "name": "Gradle line",
                      "value": "implementation 'com.kaltura.player:tvplayer:$COMMIT_SHA'"
                  }
              ]
          }
      ],
...
EOF
  ...
  DTG_VERSION=$DTG_VERSION
  DTG_PREV_VERSION=$DTG_PREV_VERSION
